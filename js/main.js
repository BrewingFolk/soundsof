...
// ── Logo rule + contributor display ────────────────────
const logoRule = document.querySelector('.logo-rule');
const logoContrib = document.getElementById('logo-contributor');

// Detect if we're on a post page and grab contributor name
const postContribName = document.querySelector('[data-post-contributor]')
  ? document.querySelector('[data-post-contributor]').dataset.postContributor
  : null;

function animateLogoIn() {
  if (postContribName) {
    // On post pages: rule animates in briefly, then swaps to contributor name
    if (logoRule) {
      logoRule.classList.add('visible');
      setTimeout(() => {
        logoRule.classList.remove('visible');
        setTimeout(() => {
          if (logoContrib) {
            logoContrib.textContent = postContribName;
            logoContrib.classList.add('visible');
          }
        }, 350);
      }, 700);
    }
  } else {
    // All other pages: rule just animates in and stays
    if (logoRule) logoRule.classList.add('visible');
  }
}

// Small delay so transition fires after paint
setTimeout(animateLogoIn, 80);

function updateLogo(contributor, playing) {
  if (!logoContrib) return;
  if (playing && contributor) {
    // Hide rule, show contributor
    if (logoRule) logoRule.classList.remove('visible');
    setTimeout(() => {
      logoContrib.textContent = contributor;
      logoContrib.classList.add('visible');
    }, 350);
  } else {
    // Hide contributor, restore rule
    logoContrib.classList.remove('visible');
    setTimeout(() => {
      if (!logoContrib.classList.contains('visible')) logoContrib.textContent = '';
      if (!postContribName && logoRule) logoRule.classList.add('visible');
      if (postContribName && logoRule) {
        // Back on post page — show contributor name again
        logoContrib.textContent = postContribName;
        logoContrib.classList.add('visible');
      }
    }, 400);
