...
app.use((req,res,next)=>{
  ...
  res.locals.siteDescription = readJSON('settings.json').siteDescription || '';
  next();
});


app.use(session({
  secret: process.env.SESSION_SECRET || 'sounds-of-secret-' + Math.random(),
  resave: false,
...
app.post('/admin/login', (req, res) => {
  ...
  if (req.body.password === (settings.adminPassword || 'brewingfolk')) {
...
// PUBLIC ROUTES
app.get('/',(req,res)=>{
  const allPosts = readJSON('posts.json').sort((a,b)=>new Date(b.publishedDate)-new Date(a.publishedDate));
  const settings = readJSON('settings.json');
  const contributors = readJSON('contributors.json');
  const perPage = settings.maxPostsOnHome || 20;
  const page = Math.max(1, parseInt(req.query.page) || 1);
  const listed = allPosts.slice((page-1)*perPage, page*perPage);
  const featured = page === 1 ? allPosts.slice(0,1) : [];
  const hasOlder = page * perPage < allPosts.length;
  const hasNewer = page > 1;
  res.render('index',{featured, listed, contributors, page, hasOlder, hasNewer});
});
...
app.get('/post/:slug',(req,res)=>{
  ...
  const contributors = readJSON('contributors.json');
  const contributor = contributors.find(c=>c.id===post.contributorId);
  const related = contributor ? posts.filter(p=>p.contributorId===contributor.id&&p.id!==post.id).sort((a,b)=>new Date(b.publishedDate)-new Date(a.publishedDate)).slice(0,8) : [];
  res.render('post',{post,contributor,related,contributors});
});
...
app.get('/category/:slug',(req,res)=>{
  ...
  const contributors = readJSON('contributors.json');
  res.render('archive',{posts,heading:label,contributors});
});
...
app.get('/tag/:slug',(req,res)=>{
  ...
  const contributors = readJSON('contributors.json');
  res.render('archive',{posts,heading:tagLabel,contributors});
});
...
app.get('/contributors',(req,res)=>{
  const contributors = readJSON('contributors.json');
  const posts = readJSON('posts.json');
  const withCount = contributors.map(c => ({...c, showCount: posts.filter(p=>p.contributorId===c.id).length}));
  res.render('contributors',{contributors:withCount});
});
...
app.get('/admin/posts/new', requireAdmin, (req,res)=>{ res.render('admin/post-form',{post:null,contributors:readJSON('contributors.json')}); });

app.post('/admin/posts/new', requireAdmin, upload.single('image'),(req,res)=>{
  const posts = readJSON('posts.json');
...
app.get('/admin/contributors/new', requireAdmin, (req,res)=>res.render('admin/contributor-form',{contributor:null}));

app.post('/admin/contributors/new', requireAdmin, upload.single('image'),(req,res)=>{
  const contributors=readJSON('contributors.json');
...
app.get('/admin/contributors/:id/edit', requireAdmin, (req,res)=>{
...
app.post('/admin/contributors/:id/edit', requireAdmin, upload.single('image'),(req,res)=>{
  const contributors=readJSON('contributors.json');
...
app.post('/admin/contributors/:id/delete', requireAdmin, (req,res)=>{
  writeJSON('contributors.json',readJSON('contributors.json').filter(c=>c.id!==req.params.id));
  res.redirect('/admin');
});

app.get('/admin/pages', requireAdmin, (req,res)=>res.render('admin/pages',{pages:readJSON('pages.json')}));

app.post('/admin/pages/about', requireAdmin, (req,res)=>{
...
app.post('/admin/pages/contact', requireAdmin, (req,res)=>{
  const pages=readJSON('pages.json');
...
app.get('/admin/settings', requireAdmin, (req,res)=>res.render('admin/settings',{settings:readJSON('settings.json')}));

app.post('/admin/settings', requireAdmin, (req,res)=>{
  const current = readJSON('settings.json');
  const updated = {siteName:req.body.siteName,siteTagline:req.body.siteTagline,maxPostsOnHome:parseInt(req.body.maxPostsOnHome)||20,logoText:req.body.logoText,siteDescription:req.body.siteDescription||'',brandLine:req.body.brandLine||'',adminPassword:req.body.adminPassword&&req.body.adminPassword.trim()?req.body.adminPassword.trim():(current.adminPassword||'brewingfolk')};
